...
const RepairPDF = ({ products }: { products: Product[] }) => {
  ...
  const columns = [
    ...
    { header: '批次', key: 'color', style: styles.cellColor },
    ...
    { header: '吹鍚植球', key: 'cpu', style: styles.cellCPU },
    { header: '開機', key: 'memory', style: styles.cellMemory },
  ...
  const renderPageContent = (pageProducts: Product[]) => (
    <>
      ...
      <Text style={styles.mainTitle}>{'維修，測試及數據清除表'}</Text>
      <View style={styles.table}>
        ...
        <View style={[styles.tableRow, styles.tableHeader]}>
          ...
          <Text style={[styles.tableCell, styles.headerCell, styles.cellColor, styles.firstHeaderCell]}></Text>
          {/* Spanning "外觀分類" */}
          <Text
            style={[
        ...
        {pageProducts.map((product, i) => (
          <View key={i} style={styles.tableRow}>
            ...
            <Text style={[styles.tableCell, styles.cellColor]}>{'1'}</Text>
            <Text style={[styles.tableCell, styles.cellDesc]}>{'DETE'}</Text>
            <Text style={[styles.tableCell, styles.cellStatus]}>{'V'}</Text>
            <Text style={[styles.tableCell, styles.cellDents]}>{'X'}</Text>
            <Text style={[styles.tableCell, styles.cellCategory]}>{'C5'}</Text>
            <Text style={[styles.tableCell, styles.cellCPU]}>{'V'}</Text>
            <Text style={[styles.tableCell, styles.cellMemory]}>{'V'}</Text>
            <Text style={[styles.tableCell, styles.cellDataClear]}>{''}</Text>
            <Text style={[styles.tableCell, styles.cellF]}>{'F4'}</Text>
            <Text style={[styles.tableCell, styles.cellSafety]}>{'X'}</Text>
            <Text style={[styles.tableCell, styles.cellTester]}>{'Rong'}</Text>
          </View>
        ))}
      </View>
    </>
