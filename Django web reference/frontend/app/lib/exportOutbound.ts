...
import OutboundPDF from '@/app/ui/dashboard/OutboundPDF';
import { ReactElement } from 'react';

export const exportOutbound = async (products: Product[]) => {
  ...
  try {
    ...
    const pdfComponent = OutboundPDF({ 
      products
    }) as ReactElement;
    ...
    link.href = url;
    link.download = `Outbound_report_${formatDate(products[0].date)}.pdf`;
